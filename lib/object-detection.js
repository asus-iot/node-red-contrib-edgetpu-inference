...
def inference(interpreter, labels, frame, dsize):
    ...
    _, scale = common.set_resized_input(interpreter, input_buf.size, lambda size: input_buf.resize(size, Image.ANTIALIAS))
    interpreter.invoke()
    ...
    cnt_labelmatch = 0
    objs = detect.get_objects(interpreter, CFG_THRESHOLD, scale)
    json_strings2="""{
      "objects": [
    """
    for obj in objs:
         ...
         if CFG_ISPARSE and CFG_PARSE not in label.lower():
                continue
         cnt_labelmatch += 1
...
def init_config():
    global TPU
    if CFG_TPUTYPE == 1:
        device = "usb:" + CFG_TPUNUM
    else:
        device = "pci:" + CFG_TPUNUM
    
    interpreter = edgetpu.make_interpreter(CFG_MODEL[1:], device)
