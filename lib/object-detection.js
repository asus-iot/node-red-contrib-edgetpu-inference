...
def inference(engine, labels, frame):
    ...
    if CFG_IMAGE:
        return {'payload': to_python2, 'inf_fps' : round(1/elapsedTime, 2), 'starttime': starttime , 'image':  base64_data_str}
    else:
        return {'payload': to_python2, 'inf_fps' : round(1/elapsedTime, 2), 'starttime': starttime}
...
def init_config():
    if CFG_TPUTYPE == 1:
        ...
        engine = DetectionEngine(CFG_MODEL[1:], tpu_usb[int(CFG_TPUNUM)])
    else:
        engine = DetectionEngine(CFG_MODEL[1:], "/dev/apex_" + CFG_TPUNUM[1:])
