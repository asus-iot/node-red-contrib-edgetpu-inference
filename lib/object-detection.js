...
CFG_RATIO = """ ` + config.keepratio +`"""
CFG_RATIO = {'0': False, '1': True}[CFG_RATIO[1:]]
CFG_RELATCOORD = """ ` + config.relativecoord +`"""
CFG_RELATCOORD = {'0': False, '1': True}[CFG_RELATCOORD[1:]]
...
CFG_BBSCALE = {False: 1, True: CFG_DNSCALE}[CFG_DSAFTER]
...
def inference(engine, labels, frame, dsize):
    ...
    objs = engine.detect_with_image(input_buf,
                                   threshold=CFG_THRESHOLD,
                                   keep_aspect_ratio=CFG_RATIO,
                                   relative_coord=CFG_RELATCOORD,
                                   top_k=CFG_TOPK,
    ...
    for idx, obj in enumerate(objs):
         ...
         label = labels[obj.label_id].lower()
         if CFG_ISPARSE and CFG_PARSE not in label:
                continue
         cnt_labelmatch += 1
         className = '\"className\" : \"'+labels[obj.label_id]+'\"'  
         json_strings2= json_strings2 + """
             {
               "bbox": [
                 """+ str(box[0]*CFG_BBSCALE)+ """,
                 """+ str(box[1]*CFG_BBSCALE)+ """,
                 """+ str(box[2]*CFG_BBSCALE)+ """,
                 """+ str(box[3]*CFG_BBSCALE)+ """
               ],
               """+ className + """,
               "score": """+ str(obj.score)+ """
         """
...
def init_config():
    ...
    if CFG_TPUTYPE == 1:
        ...
        engine = DetectionEngine(CFG_MODEL[1:], TPU)
    else:
        ...
        if (cvcap.isOpened()):
            ...
            while True:
                ...
                if ret == True:
                    ...
                    res_msgs = inference(engine, labels, img, dsize)
                    if res_msgs != "":
                        node.send(res_msgs)
                else:
...
`,
    ...
    RED.nodes.registerType('SZ Object Detection', PythonFunction);
};
